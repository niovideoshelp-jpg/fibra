...
export const Traffic:React.FC=()=>{
 const {W,H,p}=useBoard();const cx=p?W*.5:W*.8,cy=p?H*.79:H*.5;
 ...
 t.from(s('[data-hub]'),{scale:0,duration:.7,ease:'back.out(1.7)',svgOrigin:cx+' '+cy},0)
 ...
 points.forEach(([x,y],i)=>{t.from(s('[data-card="'+i+'"]'),{scale:0,rotation:i%2?18:-18,opacity:0,duration:.45,ease:'back.out(1.8)'},3.7+i*.8)
 .to(s('[data-card="'+i+'"]'),{x:cx-x,y:cy-y,scale:.03,rotation:i%2?90:-90,duration:1.2,ease:'power3.in'},4.65+i*.8);});
 t.to(s('[data-hub]'),{scale:2.4,duration:.7,ease:'power3.in',svgOrigin:cx+' '+cy},8.4);
 });
 return <Board ref={scope} bg={P.night}>
 {Array.from({length:19},(_,i)=>{const y=(i-9)*24;const d=p?'M-100 '+(H*.1+i*71)+'Q'+W*.45+' '+H*.5+' '+cx+' '+(cy+y):'M-100 '+(H*.12+i*40)+'Q'+W*.45+' '+H*.5+' '+cx+' '+(cy+y);return <g key={i} data-conduit><path d={d} stroke={P.blue} strokeWidth="6" fill="none"/><path data-flow d={d} pathLength={100} strokeDasharray="7 93" stroke={i%3?P.cyan:P.orange} strokeWidth="6" fill="none"/></g>;})}
