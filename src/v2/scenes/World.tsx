...
export const World:React.FC=()=>{
 const {W,H,p}=useBoard();const S=p?.86:1.34;const frame=useCurrentFrame();
 const zoom=interpolate(frame,[0,105,225,390],[1.035,1,1.04,1.015],{extrapolateRight:'clamp',easing:Easing.bezier(.42,0,.58,1)});
 const pan=p?0:interpolate(frame,[0,150,285,390],[-10,12,-12,0],{extrapolateRight:'clamp',easing:Easing.bezier(.42,0,.58,1)});
 ...
 return <Board ref={scope} bg={P.blue}><g transform={'translate('+(W/2+pan)+' '+H/2+') scale('+zoom+') translate('+(-W/2)+' '+(-H/2)+')'}>{p?<>
